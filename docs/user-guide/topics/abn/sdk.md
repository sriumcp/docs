...
```proto
// GetRoute returns a version number for the given RequestMeta (app/user ID combination)
...
The [Iter8 service](service.md) uses the [Iter8 storage](../glossary.md#iter8-storage) to record every route returned by `GetRoute`. 

### Illustration

A typical usage scenario for the `GetRoute` API is illustrated below. In this scenario, the Kubernetes application is distributed consists of a frontend component and a backend component. The backend is the app that is being A/B tested and has two versions. Whenever the frontend needs to send a request to the backend, it calls `GetRoute` to determine the backend version, and routes the backend request appropriately.
...
1. **Readiness**: The version returned by `GetRoute` is guaranteed to be [ready](../glossary.md#readiness). This enables safe and error-free updates and deletion of Kubernetes app resources. Assuming at least one version of the app is ready, `GetRoute` returns the `OK` status. Otherwise, it returns an error status.

2. **Weighted routing**: Given an app, a set of versions that are ready, and a user, `GetRoute` maps the user to a version in this set with probability proportional to the [version weight](../glossary.md#weight).

3. **User stickiness**: Given an app, a set of versions that are ready, and a user, distinct `GetRoute` calls for the user will return the same version. User stickiness is essential during A/B/n testing for correct attribution of metrics to versions and consistent end-user experience during an experiment.

## SetRoute
`SetRoute` takes as input an app ID, a user ID, and a version number, and records this route in [Iter8 storage](../glossary.md#iter8-storage). Its protobuf definition is as follows.
...
=== "SetRoute called by downstream service (frontend)"
    The distributed Kubernetes application consists of a frontend and a backend component. The backend is the app that is being A/B tested and has two versions. The requests from the frontend to the backend are routed through a service mesh, proxy, or some other networking mechanism. Whenever the frontend sends a request to the backend, it calls `SetRoute` to record the route.

    ![Downstream call](images/abn.backend.cr.png){: style="width:90%"}

=== "SetRoute called by app"
    Requests are routed to different versions of the app through a service mesh, proxy, or other networking mechanism. Whenever an app version receives a request, it calls `SetRoute` to record the route.

    ![App call](images/abn.app.cr.png){: style="width:75%"}

## WriteMetric
`WriteMetric` takes as input an app ID, a user ID, a [counter metric](../glossary.md#counter) name, and a metric value, and records the metric value in [Iter8 storage](../glossary.md#iter8-storage). Its protobuf definition is as follows.
...
The client bindings for the Iter8 SDK APIs can be generated, embedded, and invoked within a client written in [any gRPC supported language](https://grpc.io/docs/languages/). The code used in the [A/B testing tutorial](../../../tutorials/abn/abn.md) is in [this repo](https://github.com/iter8-tools/docs): you can refer to [client implementations](https://github.com/iter8-tools/docs/tree/main/samples/abn-sample/frontend) in [`Node`](https://github.com/iter8-tools/docs/tree/main/samples/abn-sample/frontend/node), [`Python`](https://github.com/iter8-tools/docs/tree/main/samples/abn-sample/frontend/python), and [`Go`](https://github.com/iter8-tools/docs/tree/main/samples/abn-sample/frontend/go). We now provide an overview of the main steps involved in client development. 
